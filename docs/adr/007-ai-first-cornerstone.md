...
Deferred (revisit post-Stage-2, each behind the primitives above): a read-only MCP server over the entity store (deny-by-default writes), a `guard` middleware where external policy engines evaluate agent-originated transactions pre-commit, and agent-facing query affordances.
