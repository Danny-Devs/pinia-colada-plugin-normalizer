...
function buildRefView(data: unknown, isDetailQuery: boolean): unknown {
  ...
  if (record.contactId != null) {
    ...
    const fields = isDetailQuery ? "{ name, email, role, status }" : "{ name, role, status }";
      ...
      <div class="columns">
        ...
        <div class="column">
          ...
          <div v-else class="metric success">
            → references point to the entity store. Each query derives only the fields it needs —
            but the entity is stored once.
          </div>
        </div>
        ...
        <div class="column">
          ...
          <template v-if="normalized">
            ...
            <div class="metric success">
              {{ Object.keys(storeSnapshot).length }}
              {{ Object.keys(storeSnapshot).length === 1 ? "entity" : "entities" }}, each stored
              once
            </div>
          </template>
